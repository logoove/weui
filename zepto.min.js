...
//js模板引擎
!function($){var _private={};_private.cache={};$.tpl=function(str,data,env){var fn=!/[^\w\-\.:]/.test(str)?_private.cache[str]=_private.cache[str]||this.get(document.getElementById(str).innerHTML):function(data,env){var i,variable=[],value=[];for(i in data){variable.push(i);value.push(data[i]);}
return(new Function(variable,fn.code)).apply(env||data,value);};fn.code=fn.code||"var $parts=[]; $parts.push('"
+str.replace(/\\/g,'\\\\').replace(/[\r\t\n]/g," ").split("<%").join("\t").replace(/(^|%>)[^\t]*/g,function(str){return str.replace(/'/g,"\\'");}).replace(/\t=(.*?)%>/g,"',$1,'").split("\t").join("');").split("%>").join("$parts.push('")
+"'); return $parts.join('');";return data?fn(data,env):fn;};$.adaptObject=function(element,defaults,option,template,plugin,pluginName){var $this=element;if(typeof option!='string'){var context=$.extend({},defaults,typeof option=='object'&&option);var isFromTpl=false;if($.isArray($this)&&$this.length&&$($this)[0].nodeName.toLowerCase()=="script"){$this=$($.tpl($this[0].innerHTML,context)).appendTo("body");isFromTpl=true;}
else if($.isArray($this)&&$this.length&&$this.selector==""){$this=$($.tpl($this[0].outerHTML,context)).appendTo("body");isFromTpl=true;}
else if(!$.isArray($this)){$this=$($.tpl(template,context)).appendTo("body");isFromTpl=true;}}
return $this.each(function(){var el=$(this);var data=el.data('fz.'+pluginName);if(!data)el.data('fz.'+pluginName,(data=new plugin(this,$.extend({},defaults,typeof option=='object'&&option),isFromTpl)));if(typeof option=='string')data[option]();})}}(window.Zepto);
/*selector*/
